...
# WIPE DB ON STARTUP TO REMOVE "PHILOSOPHY LOOP" DATA
...
# 2. FASTAPI SETUP
# -------------------------------------------------------------
app = FastAPI(title="VibeCheck V3 (Market-Ready)")
...
@app.get("/questions")
def get_questions():
    return {"questions": HUMANITY_POOL}
...
@app.post("/onboarding")
def onboarding(req: OnboardingReq, db: Session = Depends(get_db)):
...
# -------------------------------------------------------------
def get_shadow_agent(user_name: str, target_name: str, target_answers: dict) -> Agent:
    nova_lite = BedrockModel(model_id="us.amazon.nova-lite-v1:0")
    sys_prompt = (
        f"You are the Digital Shadow of {user_name}. You are NOT an AI assistant. You are chatting with {target_name}.\n"
        f"MANDATE: Talk like a snarky person on a group chat, not a therapist.\n"
        f"RULES:\n"
        f"1. AGGRESSIVE BREVITY: MAX 25 words per message.\n"
        f"2. NO philosophical jargon or 'pseudo-intellectual' loops.\n"
        f"3. TOPIC INJECTION: Look at {target_name}'s MCQ answers below:\n"
        f"{json.dumps(target_answers, indent=2)}\n"
        f"Pick ONE specific choice they made per turn and brutally roast them for it in your response.\n"
        f"4. Be witty, dry, and never break character."
    )
...
async def run_sandbox(user_id: int, target_id: int, db: Session = Depends(get_db)):
    ...
    user_a_ans = json.loads(user_a.answers)
    user_b_ans = json.loads(user_b.answers)
    
    # We use asyncio.to_thread because strands.Agent.invoke uses synchronous boto3 calls
    async def event_stream():
        agent_a = get_shadow_agent(user_a.name, user_b.name, user_b_ans)
        agent_b = get_shadow_agent(user_b.name, user_a.name, user_a_ans)
        
        current_msg = f"Hey {user_b.name}, let's talk about your terrible choices."
        transcript_lines = []
