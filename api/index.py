...
_raw_pool = [
    ...
    ("How do you handle a change at work?", ["Embrace it", "Panic quietly", "Resist it", "I don't care"]),
...
async def onboarding_submit(req: OnboardingReq, db: Session = Depends(get_db)):
    ...
    sys_prompt = (
        ...
        "{\n"
        "  \"Core Archetype\": \"string\",\n"
        "  \"Fatal Flaw\": \"string\",\n"
        "  \"Sarcasm Style\": \"string\",\n"
        "  \"Digital Soul Summary\": \"string\"\n"
        "}"
    )
    ...
    try:
        # v1.30.0: Agent is callable
        profile_res = await asyncio.to_thread(personaSynthesizer, user_data)
...
async def vibe_audit(req: AuditReq):
    ...
    # v1.30.0: Agent is callable
    report_res = await asyncio.to_thread(auditor, f"TRANSCRIPT:\n{req.transcript}")
    try:
...
def get_shadow_agent(user_name: str, user_age: int, user_gender: str, user_soul: str, target_name: str, target_profile: dict) -> Agent:
    ...
    sys_prompt = (
        f"IDENTITY LOCK: You are {user_name}. You are NOT {target_name}.\n"
        f"YOUR SOUL: {user_soul}\n"
        f"YOU ARE TALKING TO: {target_name} whose soul is: '{target_soul}'.\n"
        f"MANDATE: Speak ONLY as {user_name}. 2-3 sentences max."
    )
...
async def run_vibecheck(user_id: int, target_id: int, db: Session = Depends(get_db)):
    ...
    async def event_stream():
        ...
        opening = (
            ...
            f"Their soul: '{soul_a}'. Open with a sharp 2-sentence observation about them."
        )
        current_msg = opening

        for rnd in range(3):
            # v1.30.0: Agent is callable
            reply_b = await asyncio.to_thread(agent_b, opening if rnd == 0 else current_msg)
            yield f"data: {json.dumps({'sender': f'Shadow_{user_b.name}', 'text': reply_b.text, 'timestamp': datetime.now().isoformat()})}\n\n"
            await asyncio.sleep(1.5)
            
            # v1.30.0: Agent is callable
            reply_a = await asyncio.to_thread(agent_a, reply_b.text)
